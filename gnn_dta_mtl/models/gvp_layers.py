...
def tuple_sum(*args):
    """Sum tuples elementwise."""
    return tuple(map(sum, zip(*args)))


def tuple_cat(*args, dim=-1):
    """Concatenate tuples elementwise."""
...
def tuple_index(x, idx):
    """Index into tuple along first dimension."""
    return x[0][idx], x[1][idx]


def _norm_no_nan(x, axis=-1, keepdims=False, eps=1e-8, sqrt=True):
    """L2 norm with minimum value clamping."""
    out = torch.clamp(torch.sum(torch.square(x), axis, keepdims), min=eps)
    return torch.sqrt(out) if sqrt else out


def _split(x, nv):
    """Split merged representation back into tuple."""
...
def _merge(s, v):
    """Merge tuple into single tensor."""
...
class GVP(nn.Module):
    ...
    Args:
        in_dims: Input dimensions (n_scalar, n_vector)
        out_dims: Output dimensions (n_scalar, n_vector)
        h_dim: Hidden vector dimension
        activations: Activation functions (scalar_act, vector_act)
    ...
    def forward(self, x):
        ...
        Args:
            x: Input tuple (s, V) or single tensor if vi=0
            
        Returns:
            Output tuple (s, V) or single tensor if vo=0
        """
        if self.vi:
            s, v = x
            ...
            if self.vo:
                ...
                v = torch.transpose(v, -1, -2)
                if self.vector_gate:
                    gate = self.wsv(self.vector_act(s) if self.vector_act else s)
...
class GVPConv(MessagePassing):
    ...
    def __init__(
        ...
        n_layers: int = 3,
        module_list: Optional[nn.ModuleList] = None,
        aggr: str = "mean",
        activations: Tuple = (F.relu, torch.sigmoid),
        vector_gate: bool = False
    ):
...
class GVPConvLayer(nn.Module):
    """
    Full GVP convolution layer with residual connections.
    
    Args:
        node_dims: Node dimensions (n_scalar, n_vector)
        edge_dims: Edge dimensions (n_scalar, n_vector)
        n_message: Number of message passing layers
        n_feedforward: Number of feedforward layers
